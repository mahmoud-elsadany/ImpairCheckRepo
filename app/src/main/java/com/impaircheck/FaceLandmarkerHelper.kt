...
class FaceLandmarkerHelper(
    var minFaceDetectionConfidence: Float = FaceLandmarkerHelper.Companion.DEFAULT_FACE_DETECTION_CONFIDENCE,
    var minFaceTrackingConfidence: Float = FaceLandmarkerHelper.Companion.DEFAULT_FACE_TRACKING_CONFIDENCE,
    var minFacePresenceConfidence: Float = FaceLandmarkerHelper.Companion.DEFAULT_FACE_PRESENCE_CONFIDENCE,
    var maxNumFaces: Int = FaceLandmarkerHelper.Companion.DEFAULT_NUM_FACES,
    var currentDelegate: Int = FaceLandmarkerHelper.Companion.DELEGATE_CPU,
    var runningMode: RunningMode = RunningMode.IMAGE,
    val context: Context,
    // this listener is only used when running in RunningMode.LIVE_STREAM
    val faceLandmarkerHelperListener: FaceLandmarkerHelper.LandmarkerListener? = null
) {
    ...
    fun setupFaceLandmarker() {
        ...
        when (currentDelegate) {
            FaceLandmarkerHelper.Companion.DELEGATE_CPU -> {
                baseOptionBuilder.setDelegate(Delegate.CPU)
            }
            FaceLandmarkerHelper.Companion.DELEGATE_GPU -> {
                baseOptionBuilder.setDelegate(Delegate.GPU)
            }
        }

        baseOptionBuilder.setModelAssetPath(FaceLandmarkerHelper.Companion.MP_FACE_LANDMARKER_TASK)
        ...
        try {
            ...
            Log.e(
                FaceLandmarkerHelper.Companion.TAG, "MediaPipe failed to load the task with error: " + e
                    .message
            )
            ...
            faceLandmarkerHelperListener?.onError(
                "Face Landmarker failed to initialize. See error logs for " +
                        "details",
                FaceLandmarkerHelper.Companion.GPU_ERROR
            )
            Log.e(
                FaceLandmarkerHelper.Companion.TAG,
    ...
    ): FaceLandmarkerHelper.VideoResultBundle? {
    ...
    fun detectImage(image: Bitmap): FaceLandmarkerHelper.ResultBundle? {
        ...
        faceLandmarker?.detect(mpImage)?.also { landmarkResult ->
            ...
            return FaceLandmarkerHelper.ResultBundle(
                landmarkResult,
                inferenceTimeMs,
                image.height,
                image.width
    ...
    interface LandmarkerListener {
        ...
        fun onError(error: String, errorCode: Int = FaceLandmarkerHelper.Companion.OTHER_ERROR)
