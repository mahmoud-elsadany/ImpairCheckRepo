...
class FrameAnalyser( context: Context ,
                     ...
                     var faceAnalyserRepo: FaceAnalyserRepo

) : ImageAnalysis.Analyzer {

    private val realTimeOpts = FaceDetectorOptions.Builder()
            .setPerformanceMode( FaceDetectorOptions.PERFORMANCE_MODE_FAST ).setClassificationMode( FaceDetectorOptions.CLASSIFICATION_MODE_ALL )
            .build()
            ...
            for (face in faces) {
                ...
                try {
                    ...
                    if (maskLabel == maskDetectionModel.NO_MASK) {
                        ...
                        Log.d( "Average score for each user" , nameScoreHashmap.toString())
                       ...
                       val faceWidth= face.boundingBox.width()
                        ...
                        cameraFrameBitmap.width
                        cameraFrameBitmap.height
                        //print the face width and height and the camera frame width and height
                        val faceIsClose = faceWidth > cameraFrameBitmap.width * 0.5 && faceHeight > cameraFrameBitmap.height * 0.3
                        Log.d("face and camera","Face width: $faceWidth, Face height: $faceHeight, Camera frame width: ${cameraFrameBitmap.width}, Camera frame height: ${cameraFrameBitmap.height} Face is close: $faceIsClose")
                        Logger.log( "Face is close: $faceIsClose" )
                        val eyesOpen = face.leftEyeOpenProbability!! > 0.5 && face.rightEyeOpenProbability!! > 0.5
                        val smiling = face.smilingProbability!! > 0.5
                        Logger.log( "Eyes are open: $eyesOpen  and Smiling: $smiling" )

                        Log.d("face and camera","Eyes are open: $eyesOpen  and face is close: $faceIsClose and Smiling: $smiling")
                        Log.d( "Person identified as", " $bestScoreUserName " )

                        faceAnalyserRepo.theDetectedUser(bestScoreUserName)
